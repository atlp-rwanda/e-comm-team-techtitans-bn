...
const NotifyVendorsDeletion = cron.schedule('* * * * *', async () => {
    try {
        const products = await Products.findAll();
        ...
        for (const product of products) {
            ...
            const productExists = await Products.findOne({ where: { id: product.id } });
            if (!productExists) {
...
const notifyVendorOnProductBought = async (order) => {
    try{
      ...
      const orderdPrducts = await Products.findAll({where:{id: orders.product_id}});
      const vendor = await User.findOne({where:{id: orderdPrducts.vendorId}});
      if(vendor){
