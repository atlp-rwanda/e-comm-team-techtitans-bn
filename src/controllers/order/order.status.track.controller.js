...
export const getOrderStatus = async (req, res) => {
  try {
    ...
    if (userRole === 'buyer' && order.buyer.toString() !== roleID) {
      ...
      if (req.method === 'POST') {
        return res.status(403).json({ message: 'You do not have permission to update the order status.' });
      }
    }
    ...
    if (req.method === 'POST') {
      const { status, expectedDeliveryDate, shipped, delivered, cancelled, refunded, returned } = req.body;
