  ...
  try {
    ...
    if (vendorProduct) {
      return res.status(403).json({
        message: "You cannot create a product with the same name as an existing product you didn't create"
      });
    }
...
const deleteOneProduct = async (req, res) => {
  try {
    ...
    if (availableProduct === null) {
      ...
      res.status(200).json({
        status: 'success',
        message: 'This Product status has been removed because it is harmful to the human body, please contact the admin for more info.'
      });
    }
  } catch (error) {
    res.status(500).json({
