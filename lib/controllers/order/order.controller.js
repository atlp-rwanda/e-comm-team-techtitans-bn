...
const createOrder = async (req, res) => {
  try {
    ...
    const cartOrder = await _models.default.Order.findOne({
      where: {
        cartId,
        userId: id
      }
    ...
    if (cartOrder) {
      return res.status(403).json({
        message: "🚫 You have already ordered this product. please update your order instead."
      });
    }
    ...
    const order = await _models.default.Order.create({
      userId: id,
      cartId,
