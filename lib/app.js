...
var _password = _interopRequireDefault(
  require('./controllers/user/password.reminder'),
);
var _product = _interopRequireDefault(
  require('./controllers/product/product.expiration'),
);
...
app.use(
  ...
  _swaggerUiExpress.default.setup(_index2.default, options),
);
...
if (process.env.NODE_ENV === 'production') {
  app.use(
    _express.default.static(
      _path.default.join(__dirname1, '/views/chat/build'),
    ),
  );
  app.get('*', (req, res) => {
    res.sendFile(
      _path.default.resolve(__dirname1, 'views', 'chat', 'build', 'index.html'),
    );
  });
} else {
  app.get('/', (req, res) => {
